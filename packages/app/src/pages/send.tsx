...
const Send: NextPage = () => {
    ...
    const { fluxWalletAddress, balance, ownerWallet, fluxWalletAPI } = useFluxWallet()
    // const ethBalance = (parseInt(balance)) * 0.00000000000000001;
    // const ethBalance = ethers.utils.parseEther(balance);
    ...
    async function sendTransaction(recipient: string, amount: number) {
        try {
            const provider = new ethers.providers.InfuraProvider('goerli', '1befec0d558543c18368dc69ed8e5967');

            const contractAddress = '0x33a15964328a3419ec55f6192fccb81a3e3861e2';
            const contract = FluxWallet__factory.connect(fluxWalletAddress, provider);
            console.log(await contract.sendTransaction(recipient, amount));
    ...
    const sendTrans = async () => {
        sendTransaction(recepient, parseInt(amount))
    }
